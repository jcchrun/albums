...
class AlbumRootAdapter(
    private val listener: Listener
) : ListAdapter<Album, AlbumRootAdapter.AlbumRootViewHolder>(ContentDiffCallback()) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AlbumRootViewHolder {
        return AlbumRootViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.item_album_root, parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: AlbumRootViewHolder, position: Int) {
        ...
        holder.view.setOnClickListener { listener.onItemClick(holder.bindings.image, item) }
    }
    ...
    class AlbumRootViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        val bindings = ItemAlbumRootBinding.bind(view)
    }

    interface Listener {
        fun onItemClick(imageView: ImageView, album: Album)
    }
}